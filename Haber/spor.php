...
<div style="clear: both"></div>
    ...
    <main>
        ...
        <div id="baslik" style="width: 1100px;height:42px;border-bottom: 2px solid green;">
            <h2 style="float: left;background-color:green;color:white;font-weight:bold;font-size:40px;padding:0px 100px;width:150px;border-bottom-right-radius: 100px;border-top-left-radius:100px;border-left: 1px solid #000000;border-right:1px solid #000000;border-top: 1px solid #000000 ">SPOR</h2>
            <ul style="float: left;padding-top: 8px">
                <?php
                $takimlar=mysql_query("select * from takımlar");
                if(mysql_num_rows($takimlar)==0){
                }else{
                while($satır=mysql_fetch_array($takimlar)){
                    ?>
                <li style="margin-left:10px;width: 28px;height: 28px;float: left"><a target="_blank" href="<?php echo $satır['takim_link']?>" title="<?php echo $satır['takim_adi']?>"> <img src="img/takım_logoları/<?php echo $satır['takim_resim']?>" /></a></li>
                <?php }} ?>
            </ul>
        </div>
        <iframe style="flaot:left;margin-top:-5px" src="http://www.iddaatahmin.com/addsite.php?type=1&skin=2" align="middle" scrolling="yes" width="1100" height="50" frameborder="0" name="wsite" id="wsite"></iframe>
        ...
        <div id="slider_alani">
            ...
            <div id="slider">
                <ul class="slider">
                    <?php
                    $slider=mysql_query("select * from haberler WHERE haber_yer =1 and kategori_id=3 order by haber_id desc limit 10");
                ...
                <div class="sayfa">
                    <?php
                    $i=0;
                    $sayfa=mysql_query("select * from haberler WHERE haber_yer =1 and kategori_id=3 order by haber_id desc limit 10");
            ...
            <div id="yan_slider">
                <ul>
                    <ul class="yan_slider">
                        <?php
                        $yan_slider=mysql_query("select * from haberler where haber_yer=2 and kategori_id=3 order by haber_id desc limit 15");
        ...
        <div id="orta_alan">
            ...
            <div id="orta_alan_sol">
                <div id="usthaber_alan" style="max-width: 800px;height: auto; overflow: hidden;padding-left: 50px">
                    <h1 style="padding-left:200px;color: darkgreen;font-size: 30px;font-weight: bold;font-family: tahoma, arial, sans-serif">SPOR HABERLERİ</h1>
                    <ul style="max-width: 800px;height: auto; overflow: hidden">
                        <?php
                        $usthaber=mysql_query("SELECT * FROM  `haberler` WHERE haber_yer =1 AND kategori_id =3 order by haber_id desc LIMIT 10 ,27");
                        if(mysql_num_rows($usthaber)==0){
                        }else{
                            while($satır=mysql_fetch_array($usthaber)){
                                ?>
            ...
            <div id="orta_alan_sag">
                <iframe style="float: left" id="frmSoccers" class="frmFlash" frameborder="0" scrolling="no" marginwidth="0" marginheight="0" width="300" height="450" src='http://turklerhaber.com/common/service/Soccers.aspx'></iframe>
                <div id="en_cok_okunanlar">
                    <img src="img/icons/encokokunanlar.png" />
                    <ul>
                        <ul class="en_cok_okunanlar">
                            <?php
                            $en_cok_okunanlar = mysql_query("SELECT  * FROM  `haberler` where kategori_id=3 order by haber_id desc limit 30");
                ...
                <div id="dunden_kalanlar" style="">
                    ...
                    <ul>
                        <ul class="dunden_kalanlar">
                            <?php
                            $tarih = date("m.d.Y");
                            $dunden_kalanlar = mysql_query("SELECT  * FROM  `haberler` where ekleme_tarihi<'$tarih' and kategori_id=3 order by haber_id desc limit 30");
