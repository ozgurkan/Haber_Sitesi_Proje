...
<div style="clear: both"></div>
    ...
    <main>
        <div id="alt_alan" style="min-height:500px">
            <?php
            $Kullanici_Adi=$_SESSION['email'];
            $bilgilerim=mysql_query("select * from uyeler WHERE Kullanici_Adi='$Kullanici_Adi'");
            if(mysql_num_rows($bilgilerim)==0){
            }else{
                while($satır=mysql_fetch_array($bilgilerim)){
                    $uye_id=$satır['uye_id'];
                    ?>
                    <form action=""  method="post">
                        <table class="uye_giris_table">
                            <tr>
                                <td colspan="2" class="tdbaslik">Üye Bilgileri</td>
                            </tr>
                            <tr>
                                <td class="tdyanbaslik">E-mail:</td>
                                <td><input class="input" type="text" name="Email" value="<?php echo $satır['uye_email']; ?>"  /></td>
                            </tr>
                            <tr>
                                <td class="tdyanbaslik">Kullanıcı Adı:</td>
                                <td><input class="input" type="text" name="Kullanici_Adi" value="<?php echo $satır['Kullanici_Adi']; ?>"/></td>
                            </tr>
                            <tr>
                                <td class="tdyanbaslik">Şifre:</td>
                                <td><input class="input" type="text" name="Sifre" value="<?php echo $satır['uye_sifre']; ?>"  /></td>
                            </tr>
                            <tr>
                                <td>&nbsp;</td>
                            </tr>
                            ...
                            <tr>
                                <td>&nbsp;</td>
                                <td><input class="buton" type="submit" name="Guncelle" id="Guncelle" value="Güncelle" /></td>
                            </tr>
                            <?php
                            if($_POST){
                                /*Verileri alalım*/
                                ...
                                if($Kullanici_Adi=='' ||$Sifre=='' || $Email==''){
                                    ...
                                    <tr>
                                        <td>&nbsp;</td>
                                        <td><p class="hata_mesaj" style="visibility:visible">Lütfen boş alan bırakmayınız.</p></td>
                                    </tr>
                                <?php
                                }else{
                                    $query=mysql_query("Update `uyeler` set uye_email='$Email',Kullanici_Adi='$Kullanici_Adi',uye_sifre='$Sifre' where uye_id=$uye_id");
                                    if($query){
                                        session_start();
                                        $_SESSION['email'] = $Kullanici_Adi;
                                        echo '<div class="success">Bilgileriniz güncellendi.Anasayfaya yönlendiriliyorsunuz.</div>';
                                        header('Refresh: 3; url=index.php');
                                    }else{
                                        ?>
                                        <tr>
                                            <td>&nbsp;</td>
                                            <td><p class="hata_mesaj" style="visibility:visible">Bilgiler güncellenemedi.Lütfen tekrar deneyiniz.</p></td>
                                        </tr>
                                    <?php
                                    }
